...
for(i in seq_along(scens)){
  ...
  ls_r <- lapply(ls,rast)
  # calcula Media
  m2 <- function(x)mean(x,na.rm=FALSE)
  r_m <- Reduce(mean,ls_r)
  writeRaster(r_m,paste0("/dados/projetos_andamento/TRADEhub/trade_hub_plangea/future_biodiv_index_rasters/","bd_future_globiom_iiasa_",scens[i],".tif"),overwrite=T)
}
